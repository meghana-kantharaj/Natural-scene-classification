...
from .layer import ConvLayer, PoolLayer, DropOutLayer
...
class NeuralNet():
    def __init__(self, layers, training_params, allwts=None):
        ...
        assert input_layer_type in (InputLayer, ElasticLayer), \
            "First layer needs to be Input or Elastic "
    ...
    def append_next_layer(self):
        ...
        if curr_layer_type in (ElasticLayer,
                               ConvLayer, PoolLayer):
        ...
        if curr_layer_type is ElasticLayer:
        ...
        elif curr_layer_type is PoolLayer:
            curr_layer = curr_layer_type(tr_inpt,
                                         num_maps=num_prev_maps,
                                         in_sz=prev_out_sz,
                                         **layer_args)
        ...
        elif curr_layer_type in ( HiddenLayer,
                                 SoftmaxLayer):
    ...
    def get_test_model(self, x_data, y_data, aux_data=None, preds_feats=True):
        ...
        if preds_feats:
            outputs += self.te_layers[-1].features_and_predictions(self.y)
